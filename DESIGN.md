...
- Photo strip (`PhotoRow`) — contact-sheet filmstrip: ink band with `.sprockets` rows top and bottom, frames at 220/280/360px height keeping their own aspect ratio, `snap-x snap-mandatory scroll-pl-3` horizontal scroll (`scroll-pl` keeps the first frame off the band edge on phones); click opens the lightbox. The band is `md:w-fit md:max-w-full`, so a one-photo strip shrink-wraps on desktop while a long strip spans the content width and scrolls; on phones it bleeds to the viewport edges (`-mx-6`).
- Photo blocks (`AboutSection`) — each strip is an `article` with a header row (`lg:grid-cols-[300px_minmax(0,1fr)]`: serif `title` left, 2–3 line mono `text` right, `max-w-[60ch]`) over the strip; blocks separated by `gap-20 md:gap-28`, no rules. `PhotoNotes` wraps the block list and draws handwritten notes (`content/about` `notes`: strip / photo / side) in the gutter (`left`/`right`, needs ≥ 90px margin) or under the band (`below`); lines after the first wrap to the note's width. Under md the notes become `font-hand` captions under the strip.
- `LifeMap` — the "where i've lived" block: a `Sketch` box holding the China outline SVG (scales to its column, no min width) with the five stops as pins and an animated route; city labels are HTML (`font-mono text-[11px] sm:text-[12px]`, stage on a second line) positioned in % over the SVG so they stay legible at phone width. Right column (`lg:grid-cols-[minmax(0,1fr)_320px] xl:…460px`): serif title, italic lede, a mono stop ledger (`ol`, city — stage, hairline rows) and the paragraph.
