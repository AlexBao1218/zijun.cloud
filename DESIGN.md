...
- Photo board (`PhotoBoard`) — one contact sheet for all hobbies: an ink band with `.sprockets` top and bottom holding four blocks side by side in `3fr_2fr_2fr_2fr` columns, the sheet `aspect-[3/1]` so every column tiles its 6 rows exactly (wide block = big frame over two small; two photos = square over wide; one = portrait). Serif titles sit above the sheet and mono notes below, in the same columns. Under lg the blocks stack, each a short band with fixed-aspect frames (`-mx-6` bleed on phones). Click a frame for the lightbox.
